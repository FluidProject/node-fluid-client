...
class Response {
  ...
  get tokens()     { return this.checkProperty('tokens'); }
  ...
  performValidation(hash, signature) {
    ...
    this._resource = hash.resource;
    this._tokens   = hash.tokens && parseInt(hash.tokens);
    ...
    if (this._time) {
      ...
      this._expired = Math.abs(now - this._time) > client.config.tolerance;
      utilities.debug('Checking expiration given:%s now:%s [%s], expired=%s', this._time, now, new Date(), this._expired);
      if (this._expired)
